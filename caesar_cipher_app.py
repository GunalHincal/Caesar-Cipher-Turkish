...
# Arka plan görseli için CSS ekleme
def add_sidebar_background(image_path):
    st.markdown(
        f"""
        <style>
...
# Görsel dosya yolu (görselinizi proje klasörüne eklediğinizden emin olun)
image_path = "sidebar_background.jpg"

# Sidebar için arka plan görseli ekleme
add_sidebar_background(image_path)
...
st.write(
    "Metinlerinizi güvenli bir şekilde şifreleyin veya şifrelerini çözün!" if lang == "tr" else "Encrypt or decrypt your messages securely!")

st.write("")

# Yan sekmeler oluştur
tab1, tab2, tab3 = st.tabs(["Sezar Şifreleme Algoritması", "Şifreleme (Encode)", "Çözme (Decode)"] if lang == "tr" else ["What is Caesar Cipher?", "Encrypt", "Decrypt"])
...
with tab1:
    ...
    st.image("caesar_cipher_ring.jpeg",
             caption="Sezar Şifreleme Algoritması'nın Tarihsel Sembolü" if lang == "tr" else "Historical Symbol of Caesar Cipher",
             width=250)
    ...
    **Historical Background:** Julius Caesar employed this method during his campaigns in Gaul (modern-day France). Messages between soldiers and commanders were encrypted using a key (e.g., 3) determined by Caesar to keep them confidential. This simple yet effective technique prevented the enemies of the time from easily deciphering the communications.
    ...
    st.markdown("<h5 style='color:#32CD32;'>🔡 Türkçe Alfabe ve İndeksleme Yöntemi</h5>" if lang == "tr" else "<h5 style='color:#32CD32;'>🔡 Alphabet and Indexing Method</h5>", unsafe_allow_html=True)
    ...
    st.markdown("<h4 style='color:#FF6347;'>📝 Örnek Şifreleme İşlemi</h4>" if lang == "tr" else "<h4 style='color:#FF6347;'>📝 Example Encryption Process</h4>", unsafe_allow_html=True)
    st.write("Aşağıdaki örnekte, 'Merhaba Dünya' ifadesini 3 birim sağa kaydırarak nasıl şifrelediğimizi göreceksiniz." if lang == "tr" else "In the example below, see how the phrase 'Hello World' is shifted 3 units to the right.")
    ...
    st.markdown("<h4 style='color:#FFD700;'>⚠️ Önemli Terimler</h4>" if lang == "tr" else "<h4 style='color:#FFD700;'>⚠️ Important Terms</h4>", unsafe_allow_html=True)

    st.info("**Anahtar (Key):** Şifreleme ve çözme işlemi için kullanılan kaydırma değeri." if lang == "tr" else "**Key:** The value used for encoding and decoding.")
    st.info("**Şifrelenmiş Metin (Ciphertext):** Şifreleme algoritması ile dönüştürülmüş metin." if lang == "tr" else "**Ciphertext:** The text transformed by the encryption algorithm.")
    st.info("**Açık Metin (Plaintext):** Orijinal, şifrelenmemiş metin." if lang == "tr" else "**Plaintext:** The original, unencrypted text.")
    ...
    # Emoji ile alt bilgi
    st.caption(
        "🔍 Sezar Şifreleme yöntemi tarih boyunca basit güvenlik önlemleri için yaygın olarak kullanılmıştır." if lang == "tr" else
